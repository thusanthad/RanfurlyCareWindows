...
{
    public partial class DoctorAddEdit : Form
    {
        public Doctor Doctor { get; set; }
        protected AddEditDoctorBase _addEditDoctor;  

        public DoctorAddEdit(Doctor doctor, string addEdit )
        {
            InitializeComponent();
            Doctor = doctor;
            if (addEdit == "add")
            {
                _addEditDoctor = new AddNewDoctor(this);
            }
                
            else
            {
                _addEditDoctor = new EditDoctor(this);
            }
        }
        ...
        private void Save()
        {
            if (_addEditDoctor.ValidateInput())
            {
                try
                {
                    _addEditDoctor.Commit();
                    //DataBase db = new DoctorData();
